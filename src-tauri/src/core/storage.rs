...
fn ensure_transcript_columns(conn: &Connection) -> Result<(), String> {
  ...
  let add_column = |name: &str, decl: &str| -> Result<(), String> {
    ...
    conn
      .execute(&format!("ALTER TABLE transcripts ADD COLUMN {decl}"), [])
      .map_err(|err| err.to_string())?;
    Ok(())
  };
...
fn settings_entries(settings: &Settings) -> Vec<(&'static str, Value)> {
  vec![
    ("audio.input_device_id", json!(settings.audio.input_device_id)),
    ...
    ("hotkey.record_toggle", json!(settings.hotkeys.record_toggle)),
    ...
    ("transcription.model_dir", json!(settings.transcription.model_dir)),
    ("transcription.threads", json!(settings.transcription.threads)),
    ("transcription.language", json!(settings.transcription.language)),
    (
      "transcription.custom_vocab",
      json!(settings.transcription.custom_vocab),
    ),
    ("transcription.use_gpu", json!(settings.transcription.use_gpu)),
    (
    ...
    ("automation.paste_method", json!(settings.automation.paste_method)),
    ("storage.data_dir", json!(settings.storage.data_dir)),
    ("storage.keep_audio", json!(settings.storage.keep_audio)),
    ("storage.retention_days", json!(settings.storage.retention_days)),
...
fn apply_setting(settings: &mut Settings, key: &str, value: Value) {
  match key {
    ...
    "automation.auto_paste_enabled" => assign(&mut settings.automation.auto_paste_enabled, value),
