  ...
  loop {
    ...
    match decoded {
      AudioBufferRef::F32(buffer) => {
        ...
        sample_buf.copy_interleaved_ref(AudioBufferRef::F32(buffer));
        samples.extend_from_slice(sample_buf.samples());
      }
      other => {
        sample_rate = other.spec().rate;
        channels = other.spec().channels.count();
        let mut sample_buf = SampleBuffer::<f32>::new(other.capacity() as u64, *other.spec());
