...
#[cfg(target_os = "windows")]
pub fn maybe_hide_on_start(app: &AppHandle, state: &Mutex<AppState>, action: Option<cli::CliAction>) {
  let hide = {
    let guard = match state.lock() {
      Ok(guard) => guard,
      Err(_) => return,
  ...
  if matches!(action, Some(cli::CliAction::Show | cli::CliAction::ShowSettings)) {
    return;
  }
...
#[cfg(not(target_os = "windows"))]
pub fn maybe_hide_on_start(_app: &AppHandle, _state: &Mutex<AppState>, _action: Option<cli::CliAction>) {}
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
  ...
  if id == MENU_TOGGLE_ID {
    cli::handle_action(app, cli::CliAction::Toggle);
  } else if id == MENU_PASTE_ID {
    cli::handle_action(app, cli::CliAction::PasteLast);
  } else if id == MENU_OPEN_ID {
    cli::handle_action(app, cli::CliAction::Show);
  } else if id == MENU_SETTINGS_ID {
    cli::handle_action(app, cli::CliAction::ShowSettings);
...
fn build_menu(app: &AppHandle, state: &AppState) -> Result<Menu<tauri::Wry>, String> {
  ...
  menu.append(&close_to_tray_item).map_err(|err| err.to_string())?;
